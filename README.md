...
[![Mapping](https://img.shields.io/badge/Mappings-38%20classes%20%7C%20400%20methods-purple)]()
...
- ** Mapping 數據庫** — **38 類別 / 400+ 方法**，全面覆蓋 Minecraft 常用 API
...
// build.gradle
dependencies {
    implementation 'com.github.stanley-1028:francium-loader:v2.3.0'
}
```
...
```gradle
dependencies {
    implementation 'com.francium:francium-loader:2.3.0'
}
```
...
3. 並行加載模擬：各層並行加載時間線
4. 記憶體分析：模擬物件池與洩漏檢測
5. Mapping 數據庫：展示 38 類別 / 400 方法的跨版本映射
...
前往 [Releases 頁面](https://github.com/stanley-1028/francium-loader/releases/latest) 下載 `FranciumLoader-2.3.0.zip`（19.6 MB），包含：

```
FranciumLoader-2.3.0/
...
│   ├── MappingDatabase.java     # 38 類別 / 400+ 方法的跨版本數據庫
...
2. **MappingDatabase** 查詢多版本映射（Mojang、Yarn、SRG）— 現已收錄 **38 類別、400+ 方法**
...
v2.3.0 將數據庫從 20 類別大幅擴充至 **38 類別 / 400 方法**：

| 領域 | 涵蓋類別 | 方法數 |
|------|----------|--------|
| 方塊 & 物品 | Block, Item, ItemStack, BlockState, BlockEntity | ~60 |
| 世界 & 維度 | Level, ServerLevel, ClientLevel, DimensionType | ~45 |
| 實體 | Entity, LivingEntity, Player, Monster, Animal, ... | ~80 |
| 容器 & GUI | Container, MenuType, Inventory, Slot | ~35 |
| 核心引擎 | MinecraftServer, ServerPlayer, GameProfile | ~30 |
| NBT & 資料 | CompoundTag, ListTag, Tag, Component | ~40 |
| 註冊 & 標籤 | Registry, BuiltInRegistries, TagKey, Holder | ~30 |
| 事件 & 效果 | SoundEvent, Particle, MobEffect, Stats | ~25 |
| 網路 & 封包 | Packet, FriendlyByteBuf, Connection | ~25 |
| 其他常用 | RandomSource, ResourceLocation, AABB, Vec3 | ~30 |
| **總計** | **38 類別** | **~400** |

跨版本映射（如 Yarn ↔ Mojmap ↔ SRG 互轉）從 ~50 條增長至 **~400 條**，覆蓋率高達 **85%+** 最常用 API。
