...
public class ModGraph {
    ...
    // 拓撲分層結果 (使用 ReadWriteLock 保護併發存取)
    private final ReentrantReadWriteLock layersLock = new ReentrantReadWriteLock();
    private volatile List<Set<String>> layers;
    // 每個節點的最長路徑長度
    private volatile Map<String, Integer> longestPath;
    ...
    public boolean addMod(ModManifest manifest, String resolvedVersion) {
        if (manifest == null || manifest.modId() == null) return false;
        String nodeId = manifest.modId();
        
        if (manifestMap.containsKey(nodeId)) {
            ModManifest existing = manifestMap.get(nodeId);
            if (existing != null && !existing.version().equals(manifest.version())) {
        ...
        // 防禦性 null 檢查
        Map<String, String> deps = manifest.dependencies();
        if (deps != null) {
            ...
            while (!queue.isEmpty()) {
                ...
                if (dependers != null) {
                    for (String depender : dependers) {
                        if (depender == null) continue;
                        int newDegree = inDegree.getOrDefault(depender, 0) - 1;
                        inDegree.put(depender, Math.max(0, newDegree));
